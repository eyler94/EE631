...
def EpiLines(img_name, cameraMatrix, distCoeffs, img_num, F):
    ...
    x_lines = [-100, 640, -100, 640, -100, 640]
    y_lines = []
    if corners is not None: # if there are corners detected
        for spot, point in enumerate(corners): # why is there a ghost artifact in the first element
            ...
            a,b,c = lines[0][0]
            y_point = -int((c+a*x_lines[spot*2])/b)
            y_lines.append(y_point)
            y_point = -int((c+a*x_lines[spot*2+1])/b)
            y_lines.append(y_point)
...
if __name__ == "__main__":
    ...
    F = np.load('F.npy')
    diff_l, img_l, lines_l = EpiLines('stereo_L1',cameraMatrix_l, distCoeffs_l,1,F)
    diff_r, img_r, lines_r = EpiLines('stereo_R1',cameraMatrix_r, distCoeffs_r,2,F)
    for spot in range(3):
        cv2.line(img_l, (lines_r[0][spot*2],lines_r[1][spot*2]), (lines_r[0][spot*2+1],lines_r[1][spot*2+1]), (0,255,0), 2)
        cv2.line(img_r, (lines_l[0][spot*2],lines_l[1][spot*2]), (lines_l[0][spot*2+1],lines_l[1][spot*2+1]), (0,255,0), 2)
