...
hp_img = cv2.imread("reg.jpg")
hp_img = imutils.resize(hp_img, width=600)

cv2.imwrite("target.jpg", hp_img)

gray = cv2.cvtColor(hp_img, cv2.COLOR_BGR2GRAY)
features = cv2.goodFeaturesToTrack(gray, maxCorners=50, qualityLevel=0.1, minDistance=100)
...
cv2.imshow("harry potter", hp_img)
cv2.waitKey(0)
